...
| Owner-Led Operation | **[R-RAS-STORY-016]** Owner-created agent edits require owner authority. Generic world-created agents and Forge-imported system curation remain out of scope. |
...
- **[R-RAS-STORY-023]** Portfolio read surfaces for user-owned Realm Agent list/read are
  ...
  current-user `MASTER_OWNED` Realm Agents for the owner portfolio.
  `/api/agent/forge-imported-system/**`, `GET /api/creator/agents`, and
...
- **[R-RAS-STORY-035]** Forge-imported system-agent setting save through
  `PATCH /api/agent/forge-imported-system/agents/{agentId}/settings` belongs
  outside Realm Agent Studio and must not be exposed as an owner settings path.
