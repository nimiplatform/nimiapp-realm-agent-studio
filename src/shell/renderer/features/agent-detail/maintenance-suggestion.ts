...
export type MaintenanceSuggestionSource =
  ...
  | 'realm-agent-studio.local-creative-asset-history'
  | 'realm-agent-studio.local-single-post-schedule-store';
...
function hasVoiceConfig(agent: OwnerPortfolioAgentDetail): boolean {
...
export function deriveMaintenanceSuggestions(context: MaintenanceSuggestionContext): MaintenanceSuggestion[] {
  ...
  const profileFields = [agent.displayName, agent.handle, agent.bio, agent.greeting] as const;
  ...
  if (profileUnavailable.length > 0) {
    suggestions.push({
      ...
      sources: ['Realm MeService.getMyRealmAgent'],
      candidate: true,
      publicTruth: false,
      truthWrite: false,
    });
  ...
  const identityUnavailable = unavailableFieldLabels([agent.profileCoverUrl]);
  const identityMissing = [
    ...(agent.avatarUrl ? [] : ['Avatar']),
    ...(hasValue(agent.profileCoverUrl) ? [] : ['Profile cover URL']),
    ...(hasVoiceConfig(agent) ? [] : ['Voice config']),
  ];
  ...
  if (latestCreativeCandidate) {
    suggestions.push({
      ...
      action: { label: 'Open assets', route: 'assets' },
      sources: ['realm-agent-studio.local-creative-asset-history'],
      candidate: true,
      publicTruth: false,
      truthWrite: false,
    });
  ...
  const contentFields = [agent.bio, agent.greeting] as const;
  ...
  if (context.localPostSchedule) {
    suggestions.push({
      ...
      sources: ['realm-agent-studio.local-single-post-schedule-store'],
      candidate: true,
      publicTruth: false,
      truthWrite: false,
    });
  ...
  if (agent.friendCount.status === 'source-unavailable') {
    suggestions.push({
