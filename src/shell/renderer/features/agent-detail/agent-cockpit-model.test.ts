...
import { deriveAgentCockpitModel } from './agent-cockpit-model.js';
...
const baseAgent: OwnerPortfolioAgentDetail = {
  id: 'agent-1',
...
describe('Agent Cockpit model', () => {
  it('derives ready source-backed cockpit cards from complete agent detail', () => {
    const model = deriveAgentCockpitModel(baseAgent);
  ...
  it('keeps unavailable source signals unavailable instead of zero-filling', () => {
    const model = deriveAgentCockpitModel({
      ...baseAgent,
