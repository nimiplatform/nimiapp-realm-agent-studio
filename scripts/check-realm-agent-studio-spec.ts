#!/usr/bin/env tsx
/**
 * Spec consistency check for Realm Agent Studio.
 *
 ...
 * exist, the rule catalog enumerates every inline R-RAS-* identifier used across the
...
const RULE_ID_RE = /R-RAS-[A-Z]+-\d+/g;
...
async function checkRuleCatalogCoversInlineIds(): Promise<Finding[]> {
  ...
  for (const match of catalogText.matchAll(/^\s*-\s*(R-RAS-[A-Z]+-\d+)\s*$/gm)) {
