...
function loadConfig() {
  ...
  return {
    ...
    extensions: Array.isArray(audit.extensions) ? audit.extensions : ['.ts', '.tsx', '.html'],
...
function extractCandidatesFromLine(line, isMarkupFile) {
  const candidates = [];
  const textRegex = />\s*([^<>{]+?)\s*</g;
  const attrRegex = /\b(?:aria-label|ariaLabel|placeholder|title|alt|label|description|message|fallbackTitle|fallbackHint)\s*=\s*["'`]([^"'`]+)["'`]/g;
  const regexes = isMarkupFile ? [textRegex, attrRegex] : [attrRegex];
...
function lineUsesTranslation(line) {
  return line.includes('t(\'')
    || line.includes('t("')
    || line.includes('translateStudioCopy(')
    || line.includes('translateStudioExternalCopy(')
...
function runAudit() {
  ...
  for (const scopeDir of config.scopeDirs) {
    ...
    for (const filePath of collectFiles(absolute)) {
      ...
      seenFiles.add(rel);
      if (shouldExcludeFile(rel, config.excludePathPatterns)) continue;

      const source = readFileSync(filePath, 'utf8');
      const lines = source.split('\n');
      const isMarkupFile = ext === '.tsx' || ext === '.html';

      lines.forEach((line, index) => {
        const trimmed = line.trim();
        if (!trimmed || trimmed.startsWith('//') || lineUsesTranslation(line)) return;

        const candidates = extractCandidatesFromLine(line, isMarkupFile);
